...
def check_makler():
    ...
    try:
        ...
        for статья in статьи:
            ...
            цена_соответствие = re.search(r"([\d\s]+)\s*(uah|грн)", цена_text, re.IGNORECASE)
    ...
    try:
        ...
        for объявление in reversed(объявления):
            ...
            if чистая_ссылка not in отправленные_olx:
                отправленные_olx.add(chiстая_ссылка)
                
                # Поиск цены в тексте объявления на OLX RSS (обычно цена пишется в заголовке или начале описания)
